...
namespace Housing
{
    public class WallBulldozer : HousingTool, IWallTool
    {
        ...
        private void PaintEdge(HousingMesh mesh, Vector3Int a, Vector3Int b)
        {
            ...
            if (first.space == default || second.space == default) return;
            mesh.Set(first.tilePosition, firstFace, default);
            mesh.Set(second.tilePosition, secondFace, default);
        }
        ...
        public static bool TryParse(JObject json, out WallBulldozer result)
        {
            result = new WallBulldozer();
            return true;
        }
